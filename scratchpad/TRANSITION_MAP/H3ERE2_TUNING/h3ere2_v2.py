...
def run_item_v2(model, it, s1var="v4", s2var="a5", thresh=0.7, s3var="v1", retry_rule="keepfirst"):
...
def main_full(dataset, outpath, s1var="v4", s2var="a5", models=None, limit=None, thresh=0.7, s3var="v1"):
    items=[json.loads(l) for l in open(dataset) if l.strip()]
    if limit: items=items[:limit]
    mods=models or MODELS
    done=set()
