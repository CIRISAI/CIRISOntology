...
print("re-running", len(fails), "GLM records with max_tokens 2500", flush=True)
fixed = []
    ...
    for att in range(4):
        try:
            ...
            try:
                ...
                rec['kind'] = None
            fixed.append(rec); break
        except Exception as e:
            print(r['id'], "retry", att, e, flush=True); time.sleep(10)
    if (k+1) % 50 == 0: print(f"{k+1}/{len(fails)}", flush=True)
out = keep + fixed
open('panel2_validation.jsonl', 'w').write("\n".join(json.dumps(x) for x in out) + "\n")
nf = sum(1 for x in fixed if x['kind'] is None)
print("GLM-RERUN-DONE total", len(out), "residual parse-fails", nf, flush=True)
