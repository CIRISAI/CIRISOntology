...
SYS = ("You classify what FAMILY of change was made to a document. "
       ...
       + "\n".join(f"- {n}: {g}" for n,g in FAM) + "\nPick the single family the change belongs to." + "\n\nIMPORTANT: a change is Rules whenever it changes what is REQUIRED, PERMITTED, FORBIDDEN, prioritised, or the order/sequence of steps - even if it also mentions facts or figures. Deadlines, thresholds, obligations, approvals, and procedural reordering are Rules, not Facts.")
JSON_OPEN = '{"family":'
...
def main(model_path):
    ...
    out=f"{HERE}/encoded_fp32_v1.jsonl"
