...
THE QUESTION (answer it exactly as posed): A change to an artifact that alters what this item covers is a change of which kind?
...
results = {}
if os.path.exists('t2_raw.json'):
    results = json.load(open('t2_raw.json'))
for iid,(title,definition) in ITEMS.items():
    for m in MODELS:
        ...
        json.dump(results, open('t2_raw.json','w'), indent=1)
