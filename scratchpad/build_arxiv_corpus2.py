...
def changed_span(a,b):
    ...
    for tag,i1,i2,j1,j2 in sm.get_opcodes():
        if tag!='equal' and (i2-i1)+(j2-j1) >= 3:
            ...
            return pa,pb
    return None
CATS=['cs.CL','cs.CY','cs.DL','q-bio.NC','econ.EM','stat.ME','physics.hist-ph','math.LO','cs.HC','q-fin.GN']
import os
out=[]; seen=set()
if os.path.exists('plane_corpus/arxiv_rev.jsonl'):
    for l in open('plane_corpus/arxiv_rev.jsonl'):
        r=json.loads(l); out.append(r); seen.add(r['source_id'].split()[0])
    print('resuming from',len(out),'existing items',flush=True); target=int(sys.argv[1]) if len(sys.argv)>1 else 260
for cat in CATS:
