...
//! with a receipt and a separately validated certificate that compose exactly?
//!
//! # The construction
//!
...
/// A content reading of an anchor, before choosing a canonical denominator.
///
/// The source chart reads an `Anchor` as `ticks / 1`. A child chart reads the same
/// claimant from a receipt as `(cell * 2^32 + local) / factor`. Equality is therefore
/// cross-multiplication, not structural equality and not a floating-point tolerance.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct SpatialClaim {
    numerator: [u128; 2],
    denominator: u128,
...
impl SpatialClaim {
    fn equivalent(self, other: Self) -> bool {
        ...
        (0..2).all(|axis| match (
            self.numerator[axis].checked_mul(other.denominator),
            other.numerator[axis].checked_mul(self.denominator),
        ) {
...
/// The certificate is intentionally not the receipt. It binds the named roots, source
/// claimant, and receipt, and is valid only when the tier table independently supplies
/// the factor and the receipt returns exactly to that claimant.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct AddressCertificate {
    child: TierId,
    parent: TierId,
    source: Anchor,
    receipt: Receipt,
...
fn source_claim(anchor: Anchor) -> SpatialClaim {
    SpatialClaim {
        numerator: [anchor.ticks[0] as u128, anchor.ticks[1] as u128],
        denominator: 1,
    }
}

fn target_claim(receipt: Receipt) -> Option<SpatialClaim> {
    let mut numerator = [0_u128; 2];
    for (axis, slot) in numerator.iter_mut().enumerate() {
        *slot = (receipt.cell[axis] as u128)
            .checked_mul(Q32_SCALE)?
            .checked_add(receipt.local.ticks[axis] as u128)?;
    }
    Some(SpatialClaim {
        numerator,
        denominator: receipt.factor as u128,
    })
}

/// The R1 claim square for this specific content: reading after re-root agrees with
/// carrying the source reading by the independently named identity-on-position map.
fn claim_square(source: Anchor, receipt: Receipt) -> bool {
    target_claim(receipt)
        .is_some_and(|target| target.equivalent(source_claim(source)))
}

impl AddressCertificate {
    fn is_valid(self) -> bool {
        self.receipt.factor == address_factor(self.child, self.parent).unwrap_or(0)
            && join(self.receipt) == Some(self.source)
            && claim_square(self.source, self.receipt)
    }
}

fn certify_correspondence(
    child: TierId,
    parent: TierId,
    source: Anchor,
) -> Option<AddressCertificate> {
    let certificate = AddressCertificate {
        child,
        parent,
        source,
        receipt: correspondence(child, parent, source)?,
    };
...
/// Compose licenses only when the target root and claimant of the first leg are exactly
/// the source root and claimant of the second. The resulting certificate is revalidated
/// against the direct tier-table relation; composition does not mint authority.
fn compose_certificates(
    outer: AddressCertificate,
    inner: AddressCertificate,
) -> Option<AddressCertificate> {
    if !(outer.is_valid()
        && inner.is_valid()
        && outer.child == inner.parent
        && outer.receipt.local == inner.source)
    {
        return None;
    }
    let certificate = AddressCertificate {
        ...
        parent: outer.parent,
        source: outer.source,
        receipt: compose(outer.receipt, inner.receipt)?,
    };
    certificate.is_valid().then_some(certificate)
}
...
fn main() {
    ...
    let sandbox_to_grain = certify_correspondence(TierId::Grain, TierId::Sandbox, anchor)
        .expect("sandbox -> grain certified address correspondence");
    let grain_to_crystal = certify_correspondence(
        TierId::Crystal,
        TierId::Grain,
        sandbox_to_grain.receipt.local,
    )
    .expect("grain -> crystal certified address correspondence");
    let composed = compose_certificates(sandbox_to_grain, grain_to_crystal)
        .expect("composed sandbox -> crystal certificate");
    let direct = certify_correspondence(TierId::Crystal, TierId::Sandbox, anchor)
        .expect("direct sandbox -> crystal certified address correspondence");
    ...
    println!(
        ...
        sandbox_to_grain.receipt.factor,
        grain_to_crystal.receipt.factor,
        direct.receipt.factor
    );
    println!("anchor:   {:?}", anchor);
    println!("composed: {:?}", composed.receipt);
    println!("direct:   {:?}", direct.receipt);

    if composed != direct {
        eprintln!("KILLED: the named two-step certified transport is path-dependent");
        std::process::exit(1);
    }
    ...
    println!("FLAT: direct == composed; the Q32 claim square and certificate both close");
    println!("SCOPE: spatial claim only; runtime physical-state re-root still resets");
}
...
mod tests {
    ...
    #[test]
    fn the_spatial_claim_square_and_certificate_compose() {
        for anchor in ANCHORS {
            let sg = certify_correspondence(TierId::Grain, TierId::Sandbox, anchor).unwrap();
            let gc = certify_correspondence(
                TierId::Crystal,
                TierId::Grain,
                sg.receipt.local,
            )
            .unwrap();
            let via_grain = compose_certificates(sg, gc).unwrap();
            let direct =
                certify_correspondence(TierId::Crystal, TierId::Sandbox, anchor).unwrap();

            assert!(sg.is_valid());
            assert!(gc.is_valid());
            assert!(via_grain.is_valid());
            assert_eq!(via_grain, direct, "certificate path moved at {anchor:?}");
            assert!(claim_square(anchor, via_grain.receipt));
        }
    }
    ...
    #[test]
    fn a_receipt_cannot_mint_its_certificate() {
        let anchor = Anchor::from_ticks(0x1357_9bdf, 0x2468_ace0);
        let honest = certify_correspondence(TierId::Grain, TierId::Sandbox, anchor).unwrap();

        let wrong_factor = AddressCertificate {
            receipt: Receipt {
                factor: honest.receipt.factor + 1,
                ..honest.receipt
            },
            ..honest
        };
        assert!(!wrong_factor.is_valid());

        let wrong_claimant = AddressCertificate {
            source: Anchor::from_ticks(anchor.ticks[0] + 1, anchor.ticks[1]),
            ..honest
        };
        assert!(!wrong_claimant.is_valid());
    }
