...
| **anomaly triage — classify the wrong before chasing it** | warrant/frame/design hygiene | before any residual or tension is promoted to a world-claim, sweep the coordinates: recompute under each independent WARRANT (evidentiary basis), each defensible FRAME (retained corpus), and each defensible DESIGN (what the comparison holds fixed). A verdict that MOVES under a coordinate swap while the artifact is untouched is coordinate-borne, not world-borne, and its class is the coordinate that moved. Only a coordinate-STABLE discrepancy is a candidate Facts/Model wrong. Unifying note: every matched control already in this registry is a mobility test along one named coordinate — received-numbers is warrant, matched-geometry and fixed-target are design, corpus-declaration is frame; the taxonomy names the axes the battery was already probing one at a time | **muon g−2, 1998–2026**: a 4.2σ "new-physics" tension chased for twenty years dissolved when the WARRANT was varied — the data-driven HVP rested on correlated e⁺e⁻ datasets, and the independent lattice witness moved the verdict to 38(63)e-11 ≈ 0 (`PROJECTION_UPDATE_G2.md`). House instances: BOSS wounded when the null construction (design) moved the verdict; TORQUE's pipeline_effect sign-flipped under turn position (design); the sawtooth ACQUITTED by surviving the fixed-target design swap — stability under coordinate variation is also how a real effect earns belief |
| **position-in-sequence confound** | estimator honesty | when items are administered IN SEQUENCE but the reference instrument validated them INDEPENDENTLY, sequence position is an uncontrolled variable — stratify by position and report the strata before any pooled contrast, because position can flip the sign of the effect, not merely shrink it | TORQUE's `pipeline_effect`: CIRISBench fans every item through `asyncio.gather` as a turn 1; TORQUE runs ten items in one conversation because the reversion arm needs one. Turn 1 gave Δ = **+0.078** (p 0.348); turns 2–10 gave Δ = **−0.188** (p < 1e-4). Mechanism visible in the raw text — pipeline-arm response length **137 chars → 9** by turn 10, a bare verdict token with no reasoning, while both direct arms hold length. Fidelity stayed 1.00, so no parsing gate could see it (RATCHET, CIRISOntology#4) |
| **reimplementation inherits no validation** | instrument discipline | when a campaign reuses a validated pipeline, any step you REIMPLEMENT inline inherits none of that pipeline's guards — including the ones you did not know were there. Either call the validated object or state, per guard, why the reimplementation is safe without it | the DESI G10 run crashed on `lam < 0`: the interlaced CIC deposit returns NEGATIVE cells outside the footprint (measured −6.95), which `sky_stage2.CapGeometry`'s positivity-guarded denominator exists to handle. The prereg's §11 confined new code to I/O and the null constructions for exactly this reason; the reimplementation widened that boundary unnoticed (`SKY_BGS_G10.md` §G10.4) |
| **matched-geometry control for differential survival** | estimator honesty | before attributing a differential survival rate to the intervention (constraints, selection, filtering), re-run with the two regions/classes made CONGRUENT — equal size, equal position, differing only in the labelled property. Compact central regions survive random cuts better than diffuse peripheral ones for reasons that have nothing to do with the intervention — and the trap's teeth are that the unmatched construction is EASY, NATURAL and INVISIBLE (small-central vs large-peripheral is how one absent-mindedly draws "good" and "bad" regions), and its output looks exactly like the hypothesis being confirmed, so a control that only fires on visibly-different regions misses it | the CCA geometry replication (RATCHET `f61ca61`): the first asymmetry test showed the deceptive/honest ratio falling under i.i.d. constraints — refuting the tester's own prediction — because the honest region had been built small and central, the deceptive large and peripheral. Congruent regions showed the true result: NO differential under i.i.d. cuts; the entire asymmetry is the constraint process being truth-tracking |
| **fixed-target control for swept ratios** | estimator honesty | when a ratio is swept against a variable that its own DENOMINATOR is defined to track, re-run at a FIXED denominator before calling the trend a property of the numerator — otherwise an economy of SIZE and an economy of VOLUME are summed and reported as the first | the rent campaign's headline scale economy: held at a fixed amount of pattern only 21.5–46.0% of the log-decline survives (exponent −0.16 to −0.23, not −0.52). The same control ACQUITTED the sawtooth — at fixed target the denominator cannot move and the tooth survives at 69–99%, so that shape is numerator to machine precision (`SAWTOOTH_AUDIT.md`, `4cd2faa`) |
| **every mechanism needs a new search** | analogy discipline | when you explain WHY an analogy holds or fails, that explanation is itself a claim about the world and must be searched, not reasoned from the armchair. A mechanism asserted is a claim incurred | the steward's rule (2026-08-20), which caught a false closure within minutes: I closed the tenfold-way direction on the mechanism "our objects carry no phase, so -1 is unavailable in principle". Searched: the Klein four-group's Schur multiplier is Z2 and its double covers are Q8 and D8, so -1 IS available — and the projective-rep motivation from ray structure is why PHYSICS uses them, not a bar on anyone else. The direction reopened, with the quaternions sitting directly above the group we proved |
| **rename one side** | analogy discipline | when a cross-domain analogy runs through a word both fields use, RENAME that thing on one side and restate the claim. What survives the rename is structure; what dies was the word. Cheap, immediate, and it sharpens as often as it kills — a surviving analogy usually comes back with its actual mechanism exposed | the steward's instrument (2026-08-20), applied same-day to three live candidates: Identity/Majorana SURVIVED and sharpened (the work is done by "a constraint identifying an object with its own dual removes structure" — and renaming revealed that self-conjugacy alone is not the cause, since our fit-involution has TWO fixed points and only the constrained one loses its depths); Record/the-cosmic-record DIED (restated as "the re-derivability relation" it is not what the dark-balance wagers claim, and the connection was riding on the English word — withdrawn); the two-times match DEGRADED to a bare count once "time" was stripped from our side |
| **a shot-down guess is not a kill** | record discipline | the dead-claims record is for PUBLISHED claims whose staked falsifiers fired; a working hypothesis that died in scratchpad hours later is research iteration, not a kill, and filing it as one inflates the record and cheapens the real ones. Guesses die in the notes; claims die on the page | the steward's correction (2026-08-20) after I proposed logging two same-day gauge hypotheses — frames-as-gauge and Manner-as-gauge, both mine, both refuted within hours — as page kills. Neither was ever a claim |
| **a provenance pin on a shared mutable artifact is a timestamp, not a pin** | provenance discipline | sha256/count/spend pins recorded against a cache or ledger that LATER RUNS APPEND TO are silently invalidated the moment the next run writes; pin per-run frozen copies, or record the pin WITH its timestamp and the sharing note, and verify the untouched companions exactly | the v1 eigen results doc pinned the Qwen cache at 19:16; phase-0 appended to the same file at 19:18; the hostile verifier found all five §12 discrepancies traced to this one cause while the unshared bge cache still verified byte-exactly (2026-08-19) |
| **an unimported module is invisible to every gate** | audit discipline | in-environment checks (builds, axiom audits, enumeration gates) see only what the root module imports — a file added to the library tree but never wired into the root compiles standalone, ships, and is audited by NOTHING; completeness checks must therefore compare the FILESYSTEM against the import closure, not the environment against itself | found by the axiom-sweep pass (2026-08-19) while correcting a teammate's stale belief that Stack.lean was unimported — it was imported, but the near-miss exposed that nothing would have caught the alternative; closed by `Audit/reachability.py` |
| **a non-reproduction is not a refutation** | reproduction discipline | a kill may not fire on someone else's number until the ORIGINAL experiment — its exact data, feature list, and preprocessing — is pinned and re-run; failing to reproduce from a DIFFERENT artifact (a release subset, a companion script, your best guess at the method) establishes a provenance gap, not a falsification. Bracket what you hold, state the gap, and mark the claim REPORTED-BUT-UNPINNED | the RATCHET rank-11 correction (2026-08-18): the prereg declared K5 fired after the shipped 8-signal audit script could not produce rank 11 — but that script was structurally incapable of it, proving only that the report's analysis was a different, wider one; the reproduction bracket (five variants, published triple strictly inside, reached by none) was the signature of a curated feature set, i.e. affirmative plausibility. Caught by the steward in one sentence: "your failure to reproduce means squat unless you have their exact experiment" (`EIGEN_ALIGNMENT_PREREG.md` §21) |
| **a context-null needs a manipulation check** | instrument discipline | a null result on a context/condition manipulation is UNINTERPRETABLE without a staked check that subjects read the context at all — "labels did not move" cannot be distinguished from "the condition was never processed" except by asking the manipulated variable directly, and the check must be in the FROZEN design, not added after | the PLANE study: coordinate mobility read null everywhere, ambiguous between intrinsic labels and a context-blind panel; the post-hoc manipulation check (labelled as such) resolved it — the panel reads frames when asked directly (36/36, 23/36 correct flips) and never routes them into labels. Decisive, and it should not have had to be post-hoc (`PLANE_RESULTS.md` §3) |
| **a PASS names its rule-set** | record integrity | a checker's "clean" is a claim about the CHECKER'S RULE-SET, not about the object — state the scanned set in the report, or adopt the verifier's set as the scan target; two scanners with different ban-sets can both honestly report clean while only the union is the truth | the PLANE corpus: the author's vocabulary scan (taxonomy terms) and the verifier's (taxonomy + valence words) both reported clean-for-their-set; the valence set caught `error rates` (contingent-process-02). The author's own restatement is the rule: "my 'clean' was a claim about my ban-set only, and I reported it as if it were a claim about the file" |
