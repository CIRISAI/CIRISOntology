...
| **optimization MANUFACTURES closure** | **measured, one substrate** — across the Basic→Enhanced→Optimized protocol ladder the witness's predictive power goes 0.07 → marginal → 0: a learned controller is trained to make the outcome independent of the hidden fiber, the control-theory face of the design-knowing repair (`repair_creates_ferro`, the maintained-holonomy lesson). Kill: a trained protocol whose witness-relevance does NOT fall |
| rent is legible only near the floor | **measured, one substrate** — initial-KE work signature (~0.5 kT) significant at W≈3.5 kT, drowned at 11 kT; and the repetition-trained protocol is flat even at the floor: trained robustness IS insensitivity to the initial witness. Kill: the derived E4′ magnitude law failing on the chained streams |
| the wild fiber carries a SLOW mode | **measured, one substrate** — the real memory's defect is flat to 3.2 ms at 10× the matched equilibrium-Langevin gauge: persistent per-trajectory heterogeneity, not oscillator phase. The single-timescale contraction model MISFITS; successor is two-component (`CHAINED_PREREG.md`) |
