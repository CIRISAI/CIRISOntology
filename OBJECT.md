...
| **conditioning, and the organization of divergence** | **proved + TIER-2 adjudicated: the laws cut to their true scope in one round** — the conditioning level law (`sum_perturb_le`'s ceiling tracked by realized divergences) survived rule 6 TWICE at early time (f=300: 0.830, 0.880 vs staked 0.8) and **DIED as staked at late time** (f=1200: 0.27/0.33 on fresh geometries, vs 0.935 on the original — dead, kept, marked); the ke-protection claim **DIED at deep settling** (warm-up 150: 1.24 not below the ensemble p25 of 0.60); and **T-organize REPLICATED on both fresh geometries with a dose-response** — momx residual 14.9 → 12.3 → 2.5 as warm-up 60 → 90 → 150: the difference field organizes into momentum-x, less the more settled the scene, and the levels miss is the SAME phenomenon (direction-specific organization must decorrelate levels from static ceilings). The `Aggregation` budget passed its third and fourth geometry (1.0022, 1.0009) and has never been breached on this engine (`OMEGA_TIER2_RESULTS.md`) |
