...
| **the COMPOSITION claim** | **FALSIFIED TWICE at tested scopes** — COMPOSITION-2 (three observational engine arms; mechanism: closure's null semantics need μ) and Ω-KILL (7/8; the light-cone arm missed on the certify_at identity artifact, third instance of the one unrepaired engine defect, with the physics left-first by 1345 frames in the labelled diagnostic). Passed along the way: all four hardware signatures with the common-driver discriminator REPLICATED, exact-zero determinism arms, K = 1.00444 on an honest instrument. The page's wording follows `OBJECT_PRIOR_ART.md`: an unusually rigorous synthesis, not a framework. Successor gate: persistent grain identity in the engine is a PREREQUISITE, not an analysis note |
