...
| emptiness — the gluing fiber is empty | contextuality | **MEASURED (QF-1, CONFIDENCE)** — on the odd cycle the programme already owned: the KCBS pentagon (= the C5 of our Bell ceiling; θ = √5). The diagonal lift's Fine null reads CF = 0 exactly; the KCBS state reads **CF = 2√5 − 4 ≈ 0.4721** (closed form, the facet violation over its normalization); and the depolarizing family's CF zero-point lands on the facet-derived crossing to 2.9e-6. Cover lesson in the freeze: stabilizer generators are classically satisfiable and cannot pose the rung — exclusivity can (`scratchpad/omega/QF1_RESULTS.md`; credits KCBS, Cabello–Severini–Winter, Fine, Abramsky–Barbosa–Mansfield) |
...
CI green from day one. The engineering debt named at the spin-out is PAID:
the QASM harness runs, externally refereed (7/7, 5/5), with the tier walls as
its load-bearing boundaries and magic priced, not forbidden — the closure
theorems above are now the operating principles of a working engine. The
repos stay decoupled; misfits report back here.
...
|---|---|---|
| **State** (re-roots) | a cycle `A → B → C → A` | **holonomy expressible** — a re-root is a CHOSEN map. Measured once: `G_∞(q) = q/(ε + qλ)` transfers to a Wilson-loop holonomy at max residual 9.8 % (`scratchpad/HOLONOMY_RENT_RESULTS.md`) |
...
measured. **Limit, CORRECTED by the atlas** (`scratchpad/atlas/ATLAS_V2_RESULTS.md`): what forbids
...
| the detector fires DIRECTIONALLY on hardware | **measured** — `ibm_marrakesh`, one-way CRX: forward 510× floor, reverse 6.25×, **88× asymmetry**, correct sign (`scratchpad/temporal-share/CLOSURE_PILOT_RESULTS.md`) |
| `independent_views_closed` holds under severe decoherence | **measured, and a confirmed advance prediction** — independent decay is a product map, so the theorem forbids a cross-residual however severe the decay; idle pair stays at floor out to 65.5 µs ≈ T1, ρ = −0.086, p = 0.92 (`scratchpad/temporal-share/TAU_SWEEP_RESULTS.md`). **Graded weak**: the prediction is near-tautological to a physicist, and it is scope-corroboration of the detector, not support for the ontology |
| the discriminating arms: reciprocal coupling | **measured, as predicted** — both marginals and the joint open (161× floor), so joint closure is NOT generic (`scratchpad/temporal-share/RESTORATION_RESULTS.md`) |
...
| **optimization MANUFACTURES closure** | **measured at TWO levels** — survival-level: witness predictive power 0.07 → marginal → 0 across the protocol ladder; and within-erasure on the fresh chained streams: trained protocols leave ~250× less within-fiber structure (C1, `scratchpad/erasure/CHAINED_RESULTS.md`). The control-theory face of the design-knowing repair. Kill: a trained protocol whose witness-relevance does NOT fall |
