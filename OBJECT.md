...
| **the COMPOSITION claim, now the TUPLE claim** | **FALSIFIED FOUR TIMES at tested scopes** — COMPOSITION-2 (μ-free semantics), Ω-KILL (certify_at identity, 7/8), Ω-KILL-2 (6/8: materialization dust, channel-structure drift), and Ω-KILL-3, the first freeze in which the tuple's never-tested faces could each fail separately (audit-admissible, 13 arms): **10 of 12 posable arms passed — the μ-face and g-face came back ALIVE and passing** (the out-of-sample mixing bound survived real kernel drift by 2.4σ at lag 64 after its lag-1 form was refused as vacuous; the GCOST §4.2 rent bracket held tightly on both qubits with clean cycle-memory; the per-grain light-cone replicated on fresh geometry at gap 843; the common-driver discriminator hit its FOURTH replication) — **and the falsification landed on the tier/view face**: N3 as staked (two freeze-defect conjuncts, D-UNITS and D-FRAME-ZERO, its physics conjunct passing) and N4's real fire (heterogeneous closure of the declared directions). The registry is at twelve; the square's laws have STILL not lost an arm whose premises held; the channel-structure drift series is at four epochs (reverse 6.25×→19.7×→137×→32.6× floor) |
...
| **conditioning, and the organization of divergence** | **proved + measured once + STAKED FORWARD (TIER-2, in flight)** — the conditioning ceiling (`sum_perturb_le`) explains cross-view divergence LEVELS (Spearman +0.996/+0.935, 67 views) and does NOT explain GROWTH (+0.05, p = 0.70): the twins' difference field ORGANIZES into momentum-x (residual 14.9 vs random p90 8.6) and avoids kinetic energy (1.46 vs p10 4.4, χ = 1 exactly — `coherence_of_nonneg` in the field). Both halves staked on two fresh relaxation stages in `OMEGA_TIER2_PREREG.md`; a fire kills its named half |
