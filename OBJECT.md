...
| **the COMPOSITION claim** | **FALSIFIED THREE TIMES at tested scopes** — COMPOSITION-2 (μ-free semantics), Ω-KILL (certify_at identity, 7/8), Ω-KILL-2 (6/8, audit-gated: materialization dust hid a VISIBLE 901-frame light-cone, and the channel's one-wayness collapsed 88×→14×→2.8× across epochs). Each round converted its miss into a registry entry (now ten); the common-driver discriminator has replicated THREE times; the square's laws have not lost an arm whose premises held. The page's wording stands: an unusually rigorous synthesis, not a framework |
