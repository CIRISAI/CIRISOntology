...
| the slow mode | **KILLED on fresh data, kept marked** — the single-erasure flatness (3.2 ms, 10× gauge) did NOT survive a drive: C2's m=1 gain is at or below floor in all four protocols. It was within-window persistence, not trajectory identity. The contraction question is answered by C1 instead: peak 0.2 ms, contracted by 0.8 ms on the cycled ensemble |
