...
, { key      := "ai-safety"
  ...
  , confidence :=
      "A bet whose one adequately-scoped test has now RUN and come back NULL. A 7B model "
      ++ "generated its OWN judge-validated deceptions (it competently deceived ~47%% of the "
      ++ "time, so the test is real), and the whole-only difference between its deceptive and "
      ++ "honest activations is consistent with ZERO — |z| under 2.5, non-replicating (the "
      ++ "two halves of the one non-zero split disagree), and the second task family reads "
      ++ "exactly zero. The advance prediction was not confirmed, so by our own rule (support "
      ++ "comes only from a confirmed prediction) NOTHING is banked: a real negative at "
      ++ "8B activation-scale. It does NOT rule out the mechanism at larger scale, nor the "
      ++ "upkeep-cost reading (untested), nor a behaviour-level signature — but the "
      ++ "cleanest test we could run did not find it. The human-deceiver counterexample "
      ++ "stands conceded in the text."
  , promote  :=
