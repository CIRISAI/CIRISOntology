...
, { key      := "ledger"
  ...
  , plain    :=
      ...
      ++ "What killed it: take a part X even on minus-one, zero, one, and a part Y equal to "
      ++ "X squared. Our meter reads exactly zero — the floor. Now swap the labels zero and "
      ++ "one on X alone: an undoable move, touching one part, peeking at nothing. The meter "
      ++ "jumps to ln 4. Swap back and it returns to zero. The move manufactured nothing "
      ++ "real — X and Y were deeply dependent all along — but our meter reads correlation "
      ++ "of the NUMBERS, and numbering is a choice. The sentence bound a true law to a "
      ++ "meter that does not keep it.\n\n"
      ++ "What survives is stated in the next claim, about the books themselves rather than "
      ++ "the meter."
  , status   := .dead
  , kill     :=
...
, { key      := "true-books"
  ...
  , plain    :=
      "The claim above died because our meter reads numbers, and numbers are a choice. The "
      ++ "law it was reaching for is about the thing beneath the numbers: the joint "
      ++ "distribution — which outcomes occur together, and how often.\n\n"
      ++ "True shared pattern lives there. Rename any part's outcomes however you like and "
      ++ "it does not move. And it cannot be created by local action: established "
      ++ "mathematics (the data-processing inequality) says no map applied to one part "
      ++ "alone — deterministic or random, with no peeking — can increase it. Only "
      ++ "interaction writes a real entry.\n\n"
      ++ "The lesson of the dead claim stays attached to this one: any particular meter is "
      ++ "an estimator of the books, not the books, and a meter can be moved by moves that "
      ++ "write nothing.\n\n"
      ++ "How sure are we? Measured, on the predecessor record — and the mathematical core "
      ++ "is not ours; it is standard information theory, borrowed and said so."
  , status   := .measured
  , kill     :=
...
, { key      := "gravity-audit"
  ...
  , plain    :=
      ...
      ++ "form we staked, that two systems whose mass-and-energy is identical at every "
      ++ "point bend space identically.\n\n"
      ++ "What killed it: an exact vacuum gravitational wave. A pp-wave spacetime has "
      ++ "stress-energy exactly zero at every point — identical to empty flat space — yet "
      ++ "its curvature is not zero, and an interferometer tells the two apart. Einstein's "
      ++ "equations fix only part of curvature from local stress-energy; the rest is free "
      ++ "data that travels. This is textbook relativity, and it was confirmed by direct "
      ++ "measurement: detectors measured strain from passing waves in vacuum, and read "
      ++ "off the masses of the source from the pattern. Gravity, as a field, does carry "
      ++ "arrangement information.\n\n"
      ++ "What survives is the next claim, about what gravity couples to."
  , status   := .dead
  , kill     :=
...
, { key      := "gravity-charge"
  ...
  , plain    :=
      "The claim above died because a field, once sourced, carries pattern outward. What "
      ++ "survives — and is among the best-tested facts in physics — is about the coupling: "
      ++ "how a thing SOURCES gravity, and how it falls.\n\n"
...
, { key      := "dark-balance-intensive"
  ...
  , plain    :=
      ...
      ++ "all — we measured that, and it holds to the last digit we can compute. So this dark "
      ++ "energy is perfectly smooth. It never clumps, never pools in one place. It shows up "
      ++ "only in the smooth, overall stretch of space — never as lumps you could catch.\n\n"
      ...
      ++ "result could take both. Worse for this one: the early DESI data leans toward a runaway "
      ++ "past, which is evidence against THIS claim, not for it. We keep both on the page and "
      ++ "let the sky decide.\n\n"
...
, { key      := "dark-balance-extensive"
  ...
  , plain    :=
      ...
      ++ "Again: dark energy is the running balance of coordination across the cosmos. But this "
      ++ "claim adds it up as a GRAND TOTAL, not per unit — and the number of things being "
      ++ "counted grows over time. As the universe forms galaxies and clumps, there are more and "
      ++ "more coordinating pieces to add in. So the total climbs while pieces keep forming, "
      ++ "reaches a high point when forming slows, and eases down after.\n\n"
      ++ "A total that climbs and then eases has a special moment: the crest. Before the crest "
      ++ "the balance was growing — dark energy the runaway kind. At the crest it holds steady. "
      ++ "After it, it fades. So this reading predicts dark energy CROSSED the key line once, at "
      ++ "a definite moment in the past. That crossing is the bet's sharp prediction: it happens "
      ++ "at redshift 0.59, give or take 0.03 — redshift marks how long ago, and bigger means "
      ++ "longer ago.\n\n"
      ++ "Now the honesty burden, and it is heavy — we state it because our own record states "
      ++ "it. Our earlier project calls this reading a retrodiction, not a clean prediction: the "
      ++ "choice to add up the grand total, instead of the disciplined per-unit way of the claim "
      ++ "before, was made AFTER we saw that the total happened to land near the DESI number. It "
      ++ "goes against our own stated discipline, which prefers per-unit.\n\n"
      ++ "Worse, this total is built from a CLUMPY thing — the count of galaxies in a region, "
      ++ "which is higher where matter is denser. A dark energy built from a clumpy count is at "
      ++ "risk of clumping itself. And clumping dark energy is close to being ruled out already: "
      ++ "measurements of how light cools while crossing the giant empty regions between "
      ++ "galaxies look like smooth dark energy, not clumpy. So this reading owes an extra piece "
      ++ "of machinery — a reason the grand total acts only on the smooth whole and never in "
      ++ "lumps — or it runs into that measurement and loses.\n\n"
      ++ "One dated experiment decides between this reading and the one before it: DESI DR3 is a "
      ++ "single test, the two point opposite ways on it, and at most one survives — a middling "
      ++ "result could take both.\n\n"
...
, { key      := "tsvf-third"
  ...
  , plain    :=
      ...
      ++ "meeting a backward state — had no published extension carrying whole-group "
      ++ "pattern, and that building one was open work.\n\n"
      ++ "What killed it: the extension exists. Multiple-time states (Aharonov, Popescu, "
      ++ "Tollaksen and Vaidman, 2009) are time-symmetric, carry forward and backward "
      ++ "boundary data, reduce to the standard two-state picture at two boundary times, "
      ++ "and carry entanglement across multiple times that is not a function of two-party "
      ++ "data. Our sentence was false when we wrote it: the work we called future had been "
      ++ "published seventeen years earlier.\n\n"
      ++ "What is actually open is stated in the next claim."
  , status   := .dead
  , kill     :=
