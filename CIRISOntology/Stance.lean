...
, { key      := "ai-safety"
  ...
  , confidence :=
      "A bet whose one adequately-scoped test has now RUN and come back a VALIDATED "
      ++ "negative. A 7B model generated its OWN judge-validated deceptions (competently, "
      ++ "~47%% of the time), and the whole-only difference between its deceptive and honest "
      ++ "activations is consistent with ZERO across splits (|z|<2.5, non-replicating; the "
      ++ "second task family reads exactly zero). Crucially — and only after this was "
      ++ "corrected — a positive control PROVES the detector had power: a planted 10 percent "
      ++ "deceptive-honest gap fires at z=-306, while no plant reads z=-0.1, so the real ~0 "
      ++ "is a true null, not a blind spot. By our own rule (support comes only from a "
      ++ "confirmed advance prediction) NOTHING is banked FOR the bet: a real negative at 8B "
      ++ "activation-scale (layer 14, this estimator). It does NOT rule out larger scale, the "
      ++ "upkeep-cost reading (untested), or a behaviour-level signature — but the cleanest "
      ++ "test we could run, with proven power, did not find it. The human-deceiver "
      ++ "counterexample stands conceded in the text."
  , promote  :=
