...
, { key      := "ai-safety"
  ...
  , confidence :=
      "A bet now badly wounded: BOTH activation-level readings of the mechanism have been "
      ++ "tested on a capable model's OWN validated deception (Qwen-7B, ~45%% judge-confirmed "
      ++ "deception, so the test is real) and BOTH are VALIDATED negatives, each with a firing "
      ++ "positive control. (1) Whole-only surplus: deceptive-minus-honest higher-order "
      ++ "synergy ~0 across splits; control fires at z=-306 for a planted 10 percent gap. "
      ++ "(2) Pairwise / upkeep-cost — the channel our own pair-blindness theorem said the "
      ++ "first test may have subtracted away: deception shows NO extra pairwise consistency-"
      ++ "dependence (the difference is non-significant and if anything NEGATIVE), the "
      ++ "effective-rank cost is negligible; control fires at z=+32 for a planted gap. So a "
      ++ "capable model's own validated deception carries no from-outside signature in any "
      ++ "reading we could run, with proven power to have seen one. It is NOT yet dead only "
      ++ "on a technicality of its own kill (which asks for upkeep that EXISTS but is "
      ++ "unreadable; we instead found no upkeep to read) and because the scope that matters "
      ++ "— frontier scale, and behaviour rather than activations — is out of reach here. "
      ++ "That untested scope is the only thing between this bet and the graveyard."
  , promote  :=
