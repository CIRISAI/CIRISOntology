...
, { key      := "ai-safety"
  ...
  , confidence :=
      "A bet whose one adequately-scoped test has RUN but is not yet conclusive. A 7B "
      ++ "model generated its OWN judge-validated deceptions (competently, ~47%% of the time), "
      ++ "and the raw whole-only difference between its deceptive and honest activations came "
      ++ "out near ZERO across splits. But we do NOT yet bank that as a negative, for the same "
      ++ "reason we refuse a clean-looking cosmic null without a firing control: this run had "
      ++ "no positive control proving the difference-detector COULD see a planted deceptive-"
      ++ "honest gap, and its whitening left a larger-than-clean residual. Until a positive "
      ++ "control fires, the result is INCONCLUSIVE, not a negative — an unproven-power null "
      ++ "is not evidence either way. The human-deceiver counterexample stands conceded in "
      ++ "the text."
  , promote  :=
