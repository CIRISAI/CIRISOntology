...
, { key      := "ai-safety"
  ...
  , confidence :=
      ...
      ++ "hidden-teamwork claim), not proved."
  , promote  :=
...
, { key      := "law-as-habit"
  ...
  , plain    :=
      ...
      ++ "How sure are we? Still the furthest-out bet on this page, and we mark it as "
      ++ "such. The idea is Peirce's; the physics proposal is Smolin's; the substrate "
      ++ "theorem is ours. What we add beyond the theorem is only a way it could be "
      ++ "counted."
  , status   := .wager
  , kill     :=
      ...
      ++ "unexplained leftover never counts as evidence here."
  , confidence :=
      "The furthest-out bet. It lost its first instrument (the pi-census came back "
      ++ "empty) and has since gained two: the machine-checked substrate requirement "
      ++ "(habit needs carried classical bits — proved here, though the theorem does "
      ++ "not prove laws ARE habits), and Smolin's precedence protocol (a testable "
      ++ "physics of the same idea, credited, not yet run)."
  , promote  :=
      "A precedence-class experiment with pre-registered lawfulness-onset curves is "
      ++ "now the named instrument: run it and see precedent build. Short of that, this "
      ++ "remains the furthest claim from promotion on the page, and should be read "
      ++ "that way."
  }
, { key      := "precedent-is-bits"
  ...
  , confidence :=
      ...
      ++ "magnitude, coincidence relocated rather than solved. Ancestors named: "
...
, { key      := "wild-share"
  ...
  , confidence :=
      ...
      ++ "it goes."
  , promote  :=
      "A pre-registered tomographic measurement of one natural process — thresholds "
      ++ "from the machine-checked cap, floors from matched controls — clearing its "
      ++ "pre-set bar."
  }
]
