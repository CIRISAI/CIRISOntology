...
The audited state itself is deliberately not restated here. An earlier revision of this
document carried a hand-written copy of the audit table, and it went stale — eleven
theorems listed while the audit had grown to cover many times that. A hand-maintained
copy of a machine's report is exactly the kind of claim that drifts, and this document
proved it on itself. The proof assistant's own output is published verbatim, regenerated
on every deploy and never edited by hand, at
[verification.md](https://cirisai.github.io/CIRISOntology/verification.md).
...
**The oldest debt is to Charles Sanders Peirce.** The rule of predesignation — that a
hypothesis must be stated before the sample is drawn — is pre-registration, published 1883.
Fallibilism — every claim held open to defeat by future observation — is the kill condition,
stated as a temperament. Abduction names what a hypothesis is *for* — generating, never
